...
class Worksheet {
  ...
  get actualColumnCount() {
    ...
    this.eachRow(row => {
      rowCounter++;
      if (rowCounter > Worksheet.maxRows) {
        if (Worksheet.bailOnMaxLimit) {
          throw new Error(`Max row count (${  Worksheet.maxRows  }) exceeded`);
        } else {
          bailed = true;
          return;
        }
      ...
      row.eachCell(({col}) => {
        if (!counts[col]) {
          counts[col] = true;
          count++;
          if (count > Worksheet.maxCols) {
            if (Worksheet.bailOnMaxLimit) {
              throw new Error(`Max column count (${  Worksheet.maxCols  }) exceeded`);
            } else {
              bailed = true;
...
// Add static defaults
Worksheet.maxRows = 2500;
Worksheet.maxCols = 100;
Worksheet.bailOnMaxLimit = false;
