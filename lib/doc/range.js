...
class Range {
  ...
  forEachAddress(cb) {
    ...
    const {maxRows, maxCols} = Worksheet;
    ...
    for (let col = this.left; col <= this.right; col++) {
      ...
      if (colCounter > maxCols) {
        if (bailOnMaxLimit) {
          throw new Error(`Max column count (${maxCols}) exceeded in Range.forEachAddress`);
        } else {
          break;
        }
      }
      ...
      for (let row = this.top; row <= this.bottom; row++) {
        ...
        if (rowCounter > maxRows) {
          if (bailOnMaxLimit) {
            throw new Error(`Max row count (${maxRows}) exceeded in Range.forEachAddress`);
          } else {
            break;
          }
        }
