...
class CSV {
  ...
  async readFile(filename, options) {
    ...
    if (typeof options.maxRows !== 'number') options.maxRows = 1000;
    if (typeof options.maxCols !== 'number') options.maxCols = 100;
    ...
    return new Promise((resolve, reject) => {
      ...
      const csvStream = fastCsv
        ...
        .on('data', data => {
          rowCount++;
          if (rowCount > maxRows) {
            if (Worksheet.bailOnMaxLimit) {
              csvStream.emit('error', new Error(`Max row count (${maxRows}) exceeded`));
            }
            // If not bailing, just stop processing further rows
            csvStream.pause();
            return;
          }
          if (data.length > maxCols) {
            if (Worksheet.bailOnMaxLimit) {
              csvStream.emit('error', new Error(`Max column count (${maxCols}) exceeded`));
            }
            // If not bailing, just stop processing further columns
            csvStream.pause();
            return;
          }
