...
class WorksheetReader extends EventEmitter {
  ...
  async *parse() {
    ...
    const maxRows = typeof options.maxRows === 'number' ? options.maxRows : 1000;
    const maxCols = typeof options.maxCols === 'number' ? options.maxCols : 100;
    let rowCount = 0;
    let maxColCount = 0;
    let emitSheet = false;
            ...
            switch (node.name) {
              ...
              case 'row':
                if (inRows) {
                  ...
                  rowCount++;
                  if (rowCount > maxRows) {
                    if (require('../../doc/worksheet').bailOnMaxLimit) {
                      throw new Error(`Max row count (${  maxRows  }) exceeded`);
                    } else {
                      return; // Stop streaming further rows
                    }
                  }
              ...
              case 'c':
                if (row) {
                  ...
                  if (address.col > maxColCount) {
                    maxColCount = address.col;
                    if (maxColCount > maxCols) {
                      if (require('../../doc/worksheet').bailOnMaxLimit) {
                        throw new Error(`Max column count (${  maxCols  }) exceeded`);
                      } else {
                        return; // Stop streaming further columns
                      }
                    }
                  }
                }
